...
producer = KafkaProducer(
    # We used 29092 here because this py file is running outside container into CLI
...
while True:
    ...
    if isinstance(event, str):
        ...
        # Above isinstance logic is to handle the 'dirty_type' "###CORRUPTED_EVENT###"
        # It will see if event which a producer sends is a string type then print "CORRUPT EVENT SENT" else normal \
        # event even if its different dirty_type
    else:
        producer.send("traffic-topic", value=event)
        print(event)

    time.sleep(random.uniform(0.5, 1.5))
    # Above logic is used to have split second or a half, break in random order between iterations, so it wont just \
    # continue spamming without any pause
